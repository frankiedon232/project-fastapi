...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(oath2.get_current_user), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    ...
    # IMPLEMENTING JOINTS WITH SQLALCHEMY
    # EXAMPLE 1 ONLY JOINTS
    '''
    posts = db.query(models.Post).join(models.Vote, models.Vote.post_id ==
                                         models.Post.id, isouter=True).group_by(models.Post.id).all()
    '''

    # EXAMPLE 2 JOINTS INCLUDING COUNT
    '''
    posts = db.query(models.Post, func.count(models.Vote.post_id)).join(models.Vote, models.Vote.post_id ==
                                                                          models.Post.id, isouter=True).group_by(models.Post.id).all()
    '''

    # EXAMPLE 3 JOINTS INCLUDING COUNT AND RENAMING THE COUNT ALIASED COLUMN to votes INSTEAD OF THE count_1 GENERATED
    '''
    posts = db.query(
        models.Post,
    ...
    ).order_by(asc(models.Post.id)).limit(limit).offset(skip).all()
...
def get_posts(id: int, db: Session = Depends(get_db), current_user: int = Depends(oath2.get_current_user)):
    ...
    print(f"Authenticated user is: {current_user.email} | {current_user.id}")
